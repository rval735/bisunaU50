...
inline bool remainingNeuronThreshold(CLPType outputG)
{
#ifdef CONTINUOUS_PARAM
    return outputG > REMAINING_NEURON_THRESHOLD || outputG < -REMAINING_NEURON_THRESHOLD;
#else
    return outputG >= REMAINING_NEURON_THRESHOLD;
#endif
}
...
CLPType execute(ushort idx,
                ...
                const global CLPType *inputG,
                global CLPType *outputG)
{
                         ...
                         global CLPType *outputG)
{
    for (ushort idx = 0; idx < INPUT_SIZE; idx++) {
        execute(idx, false, netSt, neurons, conns, inputG, outputG);
    }
                             ...
                             global CLPType *outputG)
{
    ...
    for (;;) {
        ...
        for (ushort tempRemainingCounter = 0; tempRemainingCounter <= remainingCounter; tempRemainingCounter++) {
            ...
            CLPType exOut = execute(idx, true, netSt, neurons, conns, inputG, outputG);
...
kernel void processStateG(global const CLPType * restrict inputG,
                          global CLPType * restrict outputG,
...
// The pragma "MULTIPLECU" signals the kernel it should operate in a
...
//    size_t wg = get_local_size(0);
#endif
    {
        ...
        const global CLPType *gblInput = &(inputG[inputOffset]);
        global CLPType *gblOutput = &(outputG[outputOffset]);
        global CLCell *cell = &(cells[i]);
        ushort nSize = cell->nrsSize;
        ushort cSize = cell->connSize;
...
//        } // print once the first cell
        ...
        processInputNeurons(localNSt, cell->clNrs, cell->clCons, gblInput, gblOutput);

        processCPrimerNeurons(localNSt, cell->clNrs, cell->clCons, gblInput, gblOutput);

        processControlNeurons(localNSt, cell->clNrs, cell->clCons, gblInput, gblOutput);

        processRemainingNeurons(localNSt, cell->clNrs, cell->clCons, gblInput, gblOutput);

        processOutputNeurons(localNSt, cell->clNrs, cell->clCons, gblInput, gblOutput);

//        if (i == 0) { printCell(cells, 1); }

        for (ushort j = 0; j < nSize; j++) {
            cell->clNrs[j].clPSt =  cell->clNrs[j].clSt;
            cell->clNrs[j].clSt = 0;
            cell->clNrs[j].clExc = 0;
            cell->clNrs[j].clFired = false;
        }

        for (ushort j = 0; j < cSize; j++) {
            cell->clCons[j].clPCnType = cell->clCons[j].clCnType;
            cell->clCons[j].clCnType = false;
        }
...
//FLAGS="-D CELL_POPULATION=100 -D MULTIPLECU -D INITIALBATCH=500"
//FLAGS+="-D INPUT_SIZE=2 -D OUTPUT_SIZE=3 "
//FILES=" AllGlobalState.cl "
